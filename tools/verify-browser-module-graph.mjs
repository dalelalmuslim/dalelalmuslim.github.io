...
const REQUIRED_ELEMENT_IDS = [
  ...
  'homeAzkarResumeCard',
  'homeAzkarResumeEyebrow',
  'homeAzkarResumeTitle',
  'homeAzkarResumeMeta',
  'homeAzkarResumeButton',
  'homeAzkarResumeActionText',
  'storageDegradedBanner',
  'storageDegradedBannerText',
  'runtimeHealthStatusBadge',
  'runtimeHealthSummaryText',
  'runtimeHealthStartupText',
  'runtimeHealthStorageText',
  'runtimeHealthDayCycleText',
  'runtimeHealthFeaturesText',
  'runtimeHealthLogsText',
  'runtimeDiagnosticsStatusBadge',
  'runtimeDiagnosticsSummaryText',
  'runtimeDiagnosticsUpdatedAtText',
  'runtimeDiagnosticsList',
  'copyDiagnosticsBtn',
  'supportBundleSummaryText',
  'supportBundleMetaText',
  'supportBundlePrivacyText',
  'supportBundleStatusBadge',
  'copySupportBundleBtn',
  'downloadSupportBundleBtn',
  ...SECTION_IDS
];
...
function installGlobalStubs(readyState = 'loading') {
  ...
  Object.defineProperty(globalThis, 'Blob', { value: class Blob { constructor(parts = [], options = {}) { this.parts = parts; this.type = options.type || ''; } }, configurable: true });
...
async function runScenario(readyState) {
  ...
  if (!app.runtimeHealth?.startup?.ok) {
    throw new Error(`Runtime health did not capture startup success in ${readyState} scenario.`);
  }

  const eventBusUrl = pathToFileURL(path.join(ROOT, 'js', 'app', 'events', 'app-event-bus.js')).href;
  const { appEventBus } = await import(eventBusUrl);
  appEventBus.emit('app:section-boot-failure', { sectionId: 'settings', reason: 'synthetic verifier failure' });
  await wait(0);

  if (!app.runtimeDiagnostics?.entries?.length) {
    throw new Error(`Runtime diagnostics did not record synthetic failure in ${readyState} scenario.`);
  }

  if (!String(app.getElement?.('runtimeDiagnosticsSummaryText')?.textContent || '').trim()) {
    throw new Error(`Runtime diagnostics UI did not render summary text in ${readyState} scenario.`);
  }

  const supportBundleText = app.buildRuntimeDiagnosticsReport?.();
  if (!String(supportBundleText || '').includes('SECTION-001')) {
    throw new Error(`Support bundle report did not include diagnostic code in ${readyState} scenario.`);
  }

  const diagnosticsModuleUrl = pathToFileURL(path.join(ROOT, 'js', 'app', 'health', 'runtime-diagnostics.js')).href;
  const { buildSupportBundle } = await import(diagnosticsModuleUrl);
  const beforeCopyCount = app.runtimeDiagnostics?.supportBundleCount || 0;
  const didCopy = await app.copyRuntimeDiagnostics?.();
  if (!didCopy || (app.runtimeDiagnostics?.supportBundleCount || 0) !== beforeCopyCount + 1) {
    throw new Error(`Support bundle copy flow did not update metadata after success in ${readyState} scenario.`);
  }

  const originalCopyToClipboard = app.copyToClipboard;
  app.copyToClipboard = async () => false;
  const beforeFailedCopyCount = app.runtimeDiagnostics?.supportBundleCount || 0;
  const didFailCopy = await app.copyRuntimeDiagnostics?.();
  app.copyToClipboard = originalCopyToClipboard;
  if (didFailCopy !== false || (app.runtimeDiagnostics?.supportBundleCount || 0) !== beforeFailedCopyCount) {
    throw new Error(`Support bundle copy flow updated metadata after clipboard failure in ${readyState} scenario.`);
  }

  const loggerModuleUrl = pathToFileURL(path.join(ROOT, 'js', 'shared', 'logging', 'app-logger.js')).href;
  const { appLogger } = await import(loggerModuleUrl);
  const logPayload = ['sensitive user text', { secretToken: 'abc123', nested: true }];
  appLogger.error('[Verifier] payload sanitization check', ...logPayload, new Error('synthetic error'));
  await wait(0);

  const supportBundle = buildSupportBundle(app);
  const latestLog = supportBundle?.logs?.findLast?.((entry) => entry.scope === 'Verifier' && entry.message === 'payload sanitization check')
    || [...(supportBundle?.logs || [])].reverse().find((entry) => entry.scope === 'Verifier' && entry.message === 'payload sanitization check');
  if (!latestLog || latestLog.payloadCount !== logPayload.length + 1 || Array.isArray(latestLog.payload)) {
    throw new Error(`Support bundle log sanitization failed in ${readyState} scenario.`);
  }

  const beforeDownloadCount = app.runtimeDiagnostics?.supportBundleCount || 0;
  const didDownload = app.downloadSupportBundle?.();
  if (!didDownload || (app.runtimeDiagnostics?.supportBundleCount || 0) !== beforeDownloadCount + 1) {
    throw new Error(`Support bundle download flow did not update metadata after success in ${readyState} scenario.`);
  }

  if (!String(app.getElement?.('supportBundleMetaText')?.textContent || '').trim()) {
    throw new Error(`Support bundle UI did not render metadata text in ${readyState} scenario.`);
  }
